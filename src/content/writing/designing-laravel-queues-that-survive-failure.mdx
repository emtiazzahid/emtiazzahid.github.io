...
Worth knowing that these two are separate ceilings and the earlier one wins:

<Figure
  name="queue-retry"
  caption="With this backoff, the fifth attempt would land past the hour. It never runs — retryUntil() closes the window first, and failed() fires there instead."
/>

I've watched people set `$tries = 5` and assume they'll get five attempts. They get however many fit.
